...
const { signupSchema, loginSchema, forgetSchema, createPasswordSchema } = require('../validations/auth');
...
async function loginHandler(req, res) {
    try {
        ...
        if (!isAvailabe) return res.status(404).send({ msg: "Invalid credentials." });

        const passMatch = await bcrypt.compare(password, isAvailabe.password);
        if (!passMatch) return res.status(401).send({ msg: "Invalid credentials." });
...
async function signupHandler(req, res) {
    try {
        ...
        return res.status(201).send({ msg: "Signup successful. OTP sent to email." });
    } catch (err) {
...
async function verifyOtpHandler(req, res) {
    try {
        ...
        if (user.otpExpires < Date.now())
            return res.status(400).send({ msg: "OTP expired. Please request a new one." });
...
async function logoutHandler(req, res) {
    try {
        ...
        return res.status(500).json({ msg: "Server error during logout.", error: err.message });
    }
}
...
async function forgetPasswordHandler(req, res) {
    try {
        ...
        const { email } = req.body;
        if (!email) return res.status(400).send({ msg: "Email fields is required." });
